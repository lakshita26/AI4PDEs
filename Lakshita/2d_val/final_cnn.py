...
# Upwind-Stabilization (Artificial Diffusion to kill upstream wiggles)
nu_art = 0.005       # Momentum stabilizer
alpha_art = 0.015    # Thermal stabilizer for the sharp T=1 boundary
...
class AI4CFD(nn.Module):
    ...
    def forward(self, u, uu, v, vv, p, pp, T, TT, b_uu, b_vv, b_TT):
        ...
        # Added artificial diffusion to simulate upwinding and suppress spatial wiggles
        u_pred = u + 0.5 * dt * (u * self.xadv(uu) + v * self.yadv(uu) + (nu + nu_art) * self.diff(uu))
        v_pred = v + 0.5 * dt * (u * self.xadv(vv) + v * self.yadv(vv) + (nu + nu_art) * self.diff(vv))
        T_pred = T + 0.5 * dt * (u * self.xadv(TT) + v * self.yadv(TT) + (1.0 / (Re * Pr) + alpha_art) * self.diff(TT))
        ...
        # 2. CORRECTOR STEP 
        u_star = u + dt * (u_pred * self.xadv(b_uu) + v_pred * self.yadv(b_uu) + (nu + nu_art) * self.diff(b_uu))
        v_star = v + dt * (u_pred * self.xadv(b_vv) + v_pred * self.yadv(b_vv) + (nu + nu_art) * self.diff(b_vv))
        T_new  = T + dt * (u_pred * self.xadv(b_TT) + v_pred * self.yadv(b_TT) + (1.0 / (Re * Pr) + alpha_art) * self.diff(b_TT))
...
def save_validation_plots(u_tensor, v_tensor, T_tensor, phase_name):
    ...
    # Calculate exact Stream Function (psi = integral of U dy)
    psi = np.cumsum(U_plot * dy_val, axis=0)
